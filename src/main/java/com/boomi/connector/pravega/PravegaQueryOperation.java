...
public class PravegaQueryOperation extends BaseQueryOperation implements AutoCloseable {
    ...
    protected void executeQuery(QueryRequest request, OperationResponse response) {
        ...
        try {
            logger.info(String.format("Reading all the events from %s/%s", pravegaConfig.getScope(), pravegaConfig.getStream()));
            ...
            logger.log(Level.INFO, String.format("No more events from %s/%s", pravegaConfig.getScope(), pravegaConfig.getStream()));
        } catch (Exception e) {
